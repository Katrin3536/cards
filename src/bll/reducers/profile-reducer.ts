...
export const setProfileInfoAC = (payload: LoginResponseType) => ({type: 'PROFILE/set-profile-info', payload} as const);

export const setUserNameAC = (name: string) => ({type: 'PROFILE/set-user-name', name} as const);

export const setUserAvatarAC = (avatar: string) => ({type: 'PROFILE/set-user-avatar', avatar} as const);

// ==== THUNKS =====

export const updateUserNameTC =
    (name: string): AppThunk => async dispatch => {
        try {
...
export const updateUserAvatarTC =
    (avatar: any): AppThunk => async dispatch => {
        try {
            dispatch(appSetStatusAC('loading'));
